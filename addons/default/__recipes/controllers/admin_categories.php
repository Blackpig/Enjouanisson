...
class Admin_Categories extends Admin_Controller
{
	 ...
	 * - load the recipes_categories model.
	 * - load the categories and recipes language files.
	...
	public function index()
	{
		...
		$total_rows = $this->recipes_categories_m->count_all();
		$pagination = create_pagination('admin/recipes/categories/index', $total_rows, Settings::get('records_per_page'), 5);

		// Using this data, get the relevant results
		$categories = $this->recipes_categories_m
								->order_by('title')
	...
	public function create()
	{
		...
		{
			if ($id = $this->recipes_categories_m->insert($this->input->post()))
			{
				// Fire an event. A new recipes category has been created.
				Events::trigger('recipes_category_created', $id);
	...
	{
		// Get the category
		$category = $this->recipes_categories_m->get($id);

		// ID specified?
		$category or redirect('admin/recipes/categories/index');
		...
		if ($this->form_validation->run())
		{
			$this->recipes_categories_m->update($id, $this->input->post())
			...
			// Fire an event. A recipes category is being updated.
			Events::trigger('recipes_category_updated', $id);

			redirect('admin/recipes/categories/index');
		}
	...
	public function delete($id = 0)
	{
		...
		if (!empty($id_array))
		{
			...
			{
				if ($this->recipes_categories_m->delete($id))
				{
					$deleted++;
					$deleted_ids[] = $id;
				}
			...
			Events::trigger('recipes_category_deleted', $deleted_ids);
		}
		else
		{
	...
	{
		if ($this->recipes_categories_m->check_title($title, $this->input->post('id')))
		{
	...
	{
		if ($this->recipes_categories_m->check_slug($slug, $this->input->post('id')))
		{
	...
	public function create_ajax()
	{
		...
		if ($this->form_validation->run())
		{
			$id = $this->recipes_categories_m->insert_ajax($this->input->post());
