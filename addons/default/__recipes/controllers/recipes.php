...
/**
 * Public Recipes module controller
 *
...
class Recipes extends Public_Controller
{
	 ...
	 * - load the recipes language file.
	 */
	public function __construct()
	{
		parent::__construct();
		$this->load->model('recipes_m');
		$this->load->model('recipes_categories_m');
		...
		$cates = $this->db->get('recipes_categories')->result_array();
		...
		// Get recipes stream. We use this to set the template
		// stream throughout the recipes module.
		$this->stream = $this->streams_m->get_stream('recipes', true, 'recipess');
	}

	/**
	 * Index
	 *
	 * List out the recipes posts.
	 *
	 * URIs such as `recipes/page/x` also route here.
	 */
	public function index()
	{
		// Get our comment count whil we're at it.
		$this->row_m->sql['select'][] = "(SELECT COUNT(id) FROM ".
				$this->db->protect_identifiers('comments', true)." WHERE module='recipes'
		...
		$posts = $this->streams->entries->get_entries(array(
			...
			'where'			=> "`status` = 'live'",
			'paginate'		=> 'yes',
			'pag_base'		=> site_url('recipes/page'),
			'pag_segment'   => 3
		));
		...
		$this->template
			...
			->set_breadcrumb(lang('recipes:recipes_title'))
			->set_metadata('og:title', $this->module_details['name'], 'og')
			->set_metadata('og:type', 'recipes', 'og')
	...
	{
		$slug or redirect('recipes');

		// Get category data
		$category = $this->recipes_categories_m->get_by('slug', $slug) OR show_404();

		// Get the recipes posts
		$params = array(
			'stream'		=> 'recipes',
			'namespace'		=> 'recipes',
			'limit'			=> Settings::get('records_per_page'),
		...
		$this->template->title($this->module_details['name'], $category->title)
			...
			->set_breadcrumb(lang('recipes:recipes_title'), 'recipes')
	...
	public function archive($year = null, $month = '01')
	{
		...
		$month_year = format_date($month_date->format('U'), lang('recipes:archive_date_format'));
		...
		$this->template
			->title($month_year, lang('recipes:archive_title'), lang('recipes:recipes_title'))
			->set_metadata('description', $month_year.'. '.$meta['description'])
			->set_metadata('keywords', $month_year.', '.$meta['keywords'])
			->set_breadcrumb(lang('recipes:recipes_title'), 'recipes')
			->set_breadcrumb(lang('recipes:archive_title').': '.format_date($month_date->format('U'), lang('recipes:archive_date_format')))
			->set('pagination', $posts['pagination'])
			->set_stream($this->stream->stream_slug, $this->stream->stream_namespace)
			->set('posts', $posts['entries'])
	 ...
	 * View a post
	 *
	 * @param string $slug The slug of the recipes post.
	 */
	...
	public function preview($hash = '')
	{
		...
		if ($post['status'] === 'live')
		{
			redirect('recipes/'.date('Y/m', $post['created_on']).'/'.$post['slug']);
		}
	 ...
	 * Tagged Posts
	 *
	 * Displays recipes posts tagged with a
	...
	public function tagged($tag = '')
	{
		...
		$tag = rawurldecode($tag) or redirect('recipes');
		...
		// Maybe the entire recipes moduel should be replaced with stream
		...
		$this->row_m->sql['join'][] = 'JOIN '.$this->db->protect_identifiers('keywords_applied', true).' ON '.
			...
			$this->db->protect_identifiers('recipes.keywords', true);
		...
		// Build the page
		$this->template
			->title($this->module_details['name'], lang('recipes:tagged_label').': '.$name)
			->set_metadata('description', lang('recipes:tagged_label').': '.$name.'. '.$meta['description'])
			->set_metadata('keywords', $name)
			->set_breadcrumb(lang('recipes:recipes_title'), 'recipes')
			->set_breadcrumb(lang('recipes:tagged_label').': '.$name)
			->set('pagination', $posts['pagination'])
			->set_stream($this->stream->stream_slug, $this->stream->stream_namespace)
			->set('posts', $posts['entries'])
			->set('tag', $tag)
	...
	private function _single_view($post)
	{
		...
		$this->template->set_breadcrumb(lang('recipes:recipes_title'), 'recipes');

		if ($post['category_id'] > 0)
		{
			// Get the category. We'll just do it ourselves
			// since we need an array.
			if ($category = $this->db->limit(1)->where('id', $post['category_id'])->get('recipes_categories')->row_array())
			{
				$this->template->set_breadcrumb($category['title'], 'recipes/category/'.$category['slug']);
		...
		if (Settings::get('enable_comments'))
		{
			...
			$this->load->library('comments/comments', array(
				...
				'module' => 'recipes',
				'singular' => 'recipes:post',
				'plural' => 'recipes:posts',
			));
		...
		$this->template
			->title($post['title'], lang('recipes:recipes_title'))
