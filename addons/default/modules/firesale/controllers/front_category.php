...
class Front_category extends Public_Controller
{
    ...
    {
        // get a list of all parent categopries to dispay on the products home page
        // no products are listed on this page

        $this->data->categories = $this->pyrocache->model('categories_m', 'get_parents', array(), $this->firesale->cache_time);
        $this->data->count = count($this->data->categories);
        $this->data->rows = ceil($this->data->count / 2);
        ...
        $this->template->design = 'categories';
        // Fire events
        $overload = Events::trigger('page_build', $this->template);

        // Build page
        $this->template->build(( $overload ? $overload : 'categories' ));
    ...
    public function category()
    {
        ...
        if ($category != FALSE or $category == NULL) {
            ...
            //if we have sub categories then we display them, 
            //if not then we list the products for the category
            if ($category['children'] > 0 ) {
                $kids = $this->categories_m->get_children($category['id']);
                foreach($kids as $kid) {
                    $children[] = $this->pyrocache->model('categories_m', 'get_category', array($kid), $this->firesale->cache_time); 
                }
            } else {
            // Build Query
            ...
            $this->data->categories = reassign_helper_vars($children);
