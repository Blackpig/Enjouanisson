    ...
    public function get_category($id_slug)
    {
        ...
        public function get_parents()
        {
            ...
            $params = array(
                'stream'    => 'firesale_categories',
                'namespace' => 'firesale_categories',
                'where'     => SITE_REF."_firesale_categories.parent = 0",
            ...
            if ($entries['total'] > 0) {
                foreach ($entries['entries'] as $entry) {
                    $categories[] = $this->get_category($entry['id']);
                }
    ...
    public function tree_builder($cat, $tree = '', $first = true)
    {
        ...
        if ( isset($cat['children']) ) {
            ...
            foreach ($cat['children'] as $cat) {
              ...
              // $url   = $this->pyrocache->model('routes_m', 'build_url', array('products', $cat_slug, $cat['slug']), $this->firesale->cache_time);
              // SWH - changed as url was returnoing id not url 

                $url   = "/products/" . $cat['slug'];
                $tree .= '<li id="cat_' . $cat['id'] . '">' . "\n";
                $tree .= '  <div'.( $cat['status']['key'] == '0' ? ' class="draft"' : '' ).'>' . "\n";
                $tree .= '    <a href="' . $url . '" rel="' . $cat['id'] . '">' . $cat['title'] . '</a>' . "\n";
                $tree .= '  </div>' . "\n";
