 ...
 * @package  PyroCMS\Core\Modules\Recipes\Plugins
 */
class Plugin_Recipes extends Plugin
{
	...
	public $description = array(
		'en' => 'A plugin to display information such as recipes categories and posts.',
            'fa' => 'یک پلاگین برای نمایش اطلاعاتی مانند مجموعه های بلاگ و پست ها',
        'fr' => 'Un plugin permettant d\'afficher des informations comme les catégories et articles du recipes.'
	);
		...
		$info = array(
			'posts' => array(
				'description' => array(// a single sentence to explain the purpose of this method
					'en' => 'Display recipes posts optionally filtering them by category.',
                    'fr' => 'Permet d\'afficher des articles de recipes en les filtrants par catégorie.'
				),
			...
			'categories' => array(
				'description' => array(
					'en' => 'List recipes categories.',
                    'fr' => 'Lister les catégories du recipes'
				),
				'single' => false,
				'double' => true,
			...
			'count_posts' => array(
				'description' => array(
					'en' => 'Count recipes posts that meet the conditions specified.',
                    'fr' => 'Permet de compter les articles de recipes qui remplissent certaines conditions spécifiées.'
				),
				'single' => true,
				'double' => false,
				'variables' => '',
			...
			'tags' => array(
				'description' => array(
					'en' => 'Retrieve all tags that have been applied to recipes posts.',
	 ...
	 * Recipes List
	 *
	 * Creates a list of recipes posts. Takes all of the parameters
	 * available to streams, sans stream, where, and namespace.
	 *
	 * Usage:
	 * {{ recipes:posts limit="5" }}
	 *		<h2>{{ title }}</h2>
	 * {{ /recipes:posts }}
	 *
	 * @param	array
	...
	public function posts()
	{
		...
		if ($category_string = $this->attribute('category'))
		{
			...
			{
				$cate_filter_by[] = '`'.$this->db->dbprefix('recipes_categories').'`.`'.(is_numeric($category) ? 'id' : 'slug').'` = \''.$category."'";
			}
		...
		$this->row_m->sql['select'][] = $this->db->protect_identifiers('recipes_categories.title', true)." as 'category_title'";
		$this->row_m->sql['select'][] = $this->db->protect_identifiers('recipes_categories.slug', true)." as 'category_slug'";
		$this->row_m->sql['select'][] = $this->db->protect_identifiers('recipes_categories.title', true)." as 'category||title'";
		$this->row_m->sql['select'][] = $this->db->protect_identifiers('recipes_categories.slug', true)." as 'category||slug'";
		$this->row_m->sql['join'][] = 'LEFT JOIN '.$this->db->protect_identifiers('recipes_categories', true).' ON '.$this->db->protect_identifiers('recipes_categories.id', true).' = '.$this->db->protect_identifiers('recipes.category_id', true);
		...
		{		
			...
			foreach ($posts['entries'] as &$post)
			{
				...
				$post['url'] = site_url('recipes/'.date('Y/m', $post['created_on']).'/'.$post['slug']);
		...
		// Return our content.	
		return $this->streams->parse->parse_tag_content($this->content(), $posts, 'recipes', 'recipess', $loop);
	}

	/**
	 * Categories
	 *
	 * Creates a list of recipes categories
	 *
	 * Usage:
	 * {{ recipes:categories order-by="title" limit="5" }}
	...
	public function categories()
	{
		...
		foreach ($categories as &$category)
		{
			$category->url = site_url('recipes/category/'.$category->slug);
		}
	 ...
	 *
	 * Usage:
	 * {{ recipes:count_posts author_id="1" }}
	 *
	 ...
	 * Tag/Keyword List
	 *
	 * Create a list of recipes keywords/tags
	 *
	 * Usage:
	 * {{ recipes:tags limit="10" }}
	...
	public function tags()
	{
		...
		$posts = $this->db->select('keywords')->get('recipes')->result();
