...
class Plugin_Event extends Plugin
{
	...
	public function _self_doc()
	{
		...
		$info = array(
			...
			'categories' => array(
				'description' => array(
					'en' => 'List event categories.',
                    'fr' => 'Lister les catégories du event'
				),
				'single' => false,
				'double' => true,
	 ...
	 * Event List
	 *
	 * Creates a list of event posts. Takes all of the parameters
	...
	public function posts()
	{
		...
		if ($category_string = $this->attribute('category'))
		{
			...
			{
				$cate_filter_by[] = '`'.$this->db->dbprefix('event_categories').'`.`'.(is_numeric($category) ? 'id' : 'slug').'` = \''.$category."'";
			}
		...
		$this->row_m->sql['select'][] = $this->db->protect_identifiers('event_categories.title', true)." as 'category_title'";
		$this->row_m->sql['select'][] = $this->db->protect_identifiers('event_categories.slug', true)." as 'category_slug'";
		$this->row_m->sql['select'][] = $this->db->protect_identifiers('event_categories.title', true)." as 'category||title'";
		$this->row_m->sql['select'][] = $this->db->protect_identifiers('event_categories.slug', true)." as 'category||slug'";
		$this->row_m->sql['join'][] = 'LEFT JOIN '.$this->db->protect_identifiers('event_categories', true).' ON '.$this->db->protect_identifiers('event_categories.id', true).' = '.$this->db->protect_identifiers('event.category_id', true);
		...
		// Return our content.	
		return $this->streams->parse->parse_tag_content($this->content(), $posts, 'event', 'events', $loop);
	}
	 ...
	 * Tag/Keyword List
	 *
	 * Create a list of event keywords/tags
	 *
	 * Usage:
	 * {{ event:tags limit="10" }}
	...
	public function tags()
	{
		...
		$posts = $this->db->select('keywords')->get('event')->result();
