...
class Admin extends Admin_Controller
{
	 ...
	 * - load the event and event_categories models
	...
	public function create()
	{
		...
		$stream = $this->streams->streams->get_stream('event', 'events');
		$stream_fields = $this->streams_m->get_stream_fields($stream->id, $stream->stream_namespace);

		// Get the validation for our custom event fields.
		$event_validation = $this->streams->streams->validation_array($stream->stream_slug, $stream->stream_namespace, 'new');
		
		// Combine our validation rules.
		$rules = array_merge($this->validation_rules, $event_validation);
		...
		if ($this->form_validation->run())
		{
			...
			// If the end date has not been set then we default it to the same as the start date

			if (empty($_POST['end_date'])) {

				$_POST['end_date'] = $_POST['start_date'];
				$_POST['end_date_hour'] = $_POST['start_date_hour'];
				$_POST['end_date_minute'] = $_POST['start_date_minute'];
				$_POST['end_date_am_pm'] = $_POST['start_date_am_pm'];
			}

			if ($id = $this->streams->entries->insert_entry($_POST, 'event', 'events', array('created'), $extra))
			{
	...
	public function edit($id = 0)
	{
		...
		$event_validation = array_merge($this->validation_rules, array(
			'title' => array(
				'field' => 'title',
				'label' => 'lang:global:title',
		...
		$this->form_validation->set_rules(array_merge($this->validation_rules, $event_validation));
		...
		if ($this->form_validation->run())
		{
			...
			if ($this->streams->entries->update_entry($id, $_POST, 'event', 'events', array('updated'), $extra))
			{
		...
		if ( ! empty($ids))
		{
			...
			foreach ($ids as $id)
			{
				...
				if ($post = $this->event_m->get($id))
				{
					...
					{
						$this->comment_m->where('module', 'event')->delete_by('entry_id', $id);
