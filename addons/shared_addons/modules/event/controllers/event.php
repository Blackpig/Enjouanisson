...
class Event extends Public_Controller
{
	...
	public function __construct()
	{
		...
		$cates = $this->db->get('event_categories')->result_array();
		...
		// Get event stream. We use this to set the template
		// stream throughout the event module.
		$this->stream = $this->streams_m->get_stream('event', true, 'events');
	}

	/**
	 * Index
	 *
	 * List out the event posts.
	 *
	 * URIs such as `event/page/x` also route here.
	 */
	public function index()
	{
		// Get our comment count whil we're at it.
		$this->row_m->sql['select'][] = "(SELECT COUNT(id) FROM ".
				$this->db->protect_identifiers('comments', true)." WHERE module='event'
				AND is_active='1' AND entry_key='event:post' AND entry_plural='event:posts'
				AND entry_id=".$this->db->protect_identifiers('event.id', true).") as `comment_count`";

		// Get the latest event posts
	...
	public function archive($year = null, $month = '01')
	{
		...
		$this->template
			->title($month_year, lang('event:archive_title'), lang('event:event_title'))
	 ...
	 * Tagged Posts
	 *
	 * Displays event posts tagged with a
	...
	public function tagged($tag = '')
	{
		...
		// Maybe the entire event moduel should be replaced with stream
		...
		$this->row_m->sql['join'][] = 'JOIN '.$this->db->protect_identifiers('keywords_applied', true).' ON '.
			...
			$this->db->protect_identifiers('event.keywords', true);
	...
	private function _single_view($post)
	{
		...
		$this->template->set_breadcrumb(lang('event:event_title'), 'event');

		if ($post['category_id'] > 0)
		{
			// Get the category. We'll just do it ourselves
			// since we need an array.
			if ($category = $this->db->limit(1)->where('id', $post['category_id'])->get('event_categories')->row_array())
			{
