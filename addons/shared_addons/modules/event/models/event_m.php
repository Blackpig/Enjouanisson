...
class Event_m extends MY_Model
{
	...
	{
		$this->db
			->select('event.*, event_categories.title AS category_title, event_categories.slug AS category_slug')
			...
			->join('users', 'event.author_id = users.id', 'left')
	...
	{
		return $this->db
			->select('event.*, users.username, profiles.display_name')
			->join('profiles', 'profiles.user_id = event.author_id', 'left')
			->join('users', 'event.author_id = users.id', 'left')
			->where('event.id', $id)
			->get('event')
	...
	public function get_many_by($params = array())
	{
		...
		{
			$this->db->where('MONTH(FROM_UNIXTIME('.$this->db->dbprefix('event').'.created_on))', $params['month']);
		}

		if ( ! empty($params['year']))
		{
			$this->db->where('YEAR(FROM_UNIXTIME('.$this->db->dbprefix('event').'.created_on))', $params['year']);
		}
		...
		{
			$this->db->where('event.created_on <=', now());
		}
