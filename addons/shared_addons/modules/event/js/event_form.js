(function ($) {
	$(function () {
		...
		pyro.generate_slug('#event-content-tab input[name="title"]', '#event-content-tab input[name="slug"]');
		...
		$(document.getElementById('event-options-tab')).find('ul').find('li').first().find('a').colorbox({
			srollable: false,
			innerWidth: 600,
			innerHeight: 280,
					...
					$.ajax({
						...
						success: function (obj) {
							...
							if (obj.status == 'ok') {
								...
								$(document.getElementById('event-options-tab')).find('li').first().find('span').html(obj.title);
